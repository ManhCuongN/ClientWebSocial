...
function LoginFace() {
  ...
  const faceIoScriptLoaded = () => {
    if (faceIO && !faceioInstance) {
      faceioInstance = new faceIO("fioa06b0");
    }
  };
  ...
  const faceSignIn = async () => {
    try {
      ...
      console.log("PayLoad: ",  typeof(userData));
      console.log("PayLoad: ",  userData.payload.access_token);
   ...
   // dispatch({ type: GLOBALTYPES.ALERT, payload: {loading: true} })
    ...
    dispatch({ 
        ...
        payload: {
            token: userData.payload.access_token,
            user: userData.payload.user
        }
    })
